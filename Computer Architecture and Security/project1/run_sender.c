...
int main() {
    ...
    printf("Now sending.\n");
    syncTiming(2000000000); // Making this predefined as large for ease of use during testing.
    ...
    for (int index = 0; index < 8; index++) {
        ...
        while (loops == 0 || (rdtscp() + jitter + 1) % small_interval > jitter) {
            flush(shared_file);
            loops++;    // This doesn't do anything other than (hopefully) sync with the receiver.
        }
        timeStart = init_time;
        timeEnd = rdtscp();
